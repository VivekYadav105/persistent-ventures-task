...
const Signup = ()=>{
    ...
    const {isLoading:loading,isSuccess,isError,error,mutate} = useSingupMutation();
    ...
    return(
        ...
        <div className="p-5 sm:p-8 md:p-5  lg:bg-transparent rounded-xl  lg:shadow-none">
            <article className="text-center md:mb-10">
                ...
                <p className="font-semibold mt-3">Create your account</p>
            </article>
            <form className="max-w-sm m-auto flex flex-col gap-6" onSubmit={handleSubmit(submitForm)}>
                ...
                <div className="input-field flex flex-col">
                    ...
                    <article className="w-full border-2 border-blue-400 rounded-xl bg-white flex items-center">
                        <input placeholder="Enter your password" className="bg-transparent p-2.5 rounded-lg outline-none  h-full flex-1" type={showPassword?"text":"password"} {...register('cnfrmPassword',{required:{value:true,message:"Password cannot be empty"}})}/>   
                    </article>
                    <ErrorMessage 
                        render={({message})=><span className="text-red-600 text-xs ps-2 mt-1">{message}</span>} 
                        name="cnfrmPassword" 
                ...
                <div className="input-wrapper">
                    <article className="text-xs text-center">
                        Already have an account?<button onClick={()=>navigate('/auth/login')} type="button" className="text-xs mx-1 underline text-medium text-main">Login Here</button>
                    </article>
                </div>
            </form>
        </div>
